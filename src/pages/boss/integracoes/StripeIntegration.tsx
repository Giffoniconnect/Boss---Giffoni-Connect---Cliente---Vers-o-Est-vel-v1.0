...
export default function StripeIntegration() {
    ...
    async function loadConfig() {
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (data.stripe) {
            setConfig({
              status: data.stripe.status || 'não_configurado',
              mode: data.stripe.mode || 'test',
              publishableKey: data.stripe.publishableKey || '',
              secretKeyPlaceholder: data.stripe.secretKeyPlaceholder || '',
              webhookSecretPlaceholder: data.stripe.webhookSecretPlaceholder || '',
              notes: data.stripe.notes || ''
            });
          }
        }
      } catch (err) {
        console.error('Erro ao ler Stripe de settings/connectors:', err);
      } finally {
        setLoading(false);
      }
    }
    ...
    try {
      await setDoc(doc(db, 'settings', 'connectors'), {
        stripe: {
          ...
          secretKeyPlaceholder: config.secretKeyPlaceholder.trim(),
      ...
      setFeedback({ type: 'success', message: 'Configurações de integração com Stripe salvas na nuvem com sucesso!' });
      ...
      console.error('Erro ao salvar Stripe:', err);
  ...
  const handleTestConnection = () => {
    ...
    const newLogs = [
      `[${timestamp}] Inicializando aperto de mão lógico com Stripe...`,
      `[${timestamp}] Segurança: chaves protegidas fáticas do frontend verificadas.`,
      `[${timestamp}] Conectando ao endpoint da Stripe API (${config.mode === 'live' ? 'https://api.stripe.com/v1' : 'https://api.stripe.com/v1 (Sandbox Mode)'})...`,
      ...
      <BossLayout>
        <div className="flex-1 p-6 md:p-10 flex items-center justify-center bg-gray-50 text-gray-400">
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-xs font-black uppercase tracking-widest text-gray-500">Sincronizando Stripe...</span>
          </div>
        </div>
      </BossLayout>
    );
    ...
    <BossLayout>
      <div className="flex-1 p-6 md:p-10 space-y-6 max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-3 mt-1.5">
              ...
              <div>
                <h2 className="text-lg font-black text-gray-900 tracking-tight uppercase">Stripe Gateway</h2>
                <p className="text-xs text-gray-500 font-medium">Configurações de Faturamento e Recorrência</p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-slate-900 border border-slate-950 p-6 rounded-[2rem] flex gap-3.5 text-slate-100 shadow-xl">
          ...
          <div className="space-y-1">
            <p className="text-xs font-black uppercase tracking-wider text-indigo-400 font-mono">Conector Stripe Ativo • BOSS v5</p>
            <p className="text-xs text-slate-300 leading-relaxed font-semibold">
              Processador global de faturas, cartões de crédito e custas judiciais para clientes nacionais e estrangeiros. Configure as credenciais abaixo de forma isolada.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSave} className="bg-white border border-gray-150 rounded-3xl p-6 md:p-8 space-y-6 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1 md:col-span-2">
              <label className="text-[10px] font-black uppercase text-gray-500 tracking-wider">publishableKey (Chave Pública) *</label>
              <input
                type="text"
                required
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase text-gray-500 tracking-wider">Secret Key Placeholder (Seguro) *</label>
              <input
                type="password"
                required
                value={config.secretKeyPlaceholder}
                onChange={(e) => setConfig({ ...config, secretKeyPlaceholder: e.target.value })}
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase text-gray-500 tracking-wider">Modo de Operação *</label>
              <select
                value={config.mode}
              ...
              >
                <option value="test">Sandbox / Amostra / Teste</option>
          ...
          <div className="bg-slate-900 border border-slate-950 p-5 rounded-3xl text-slate-100 font-mono text-xs space-y-3 shadow-xl">
            <div className="flex justify-between items-center border-b border-slate-800 pb-2">
              <span className="text-[10px] text-slate-400 font-black uppercase tracking-wider flex items-center gap-2">
                ...
                <span>Terminal Simulator Logs — Stripe Gateway</span>
              </span>
              <button
                type="button"
