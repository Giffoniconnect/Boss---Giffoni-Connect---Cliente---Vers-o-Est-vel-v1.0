...
interface GmailConfig {
  ...
  provider: 'smtp_sec' | 'gmail_oauth' | 'sendgrid';
  notes: string;
}
...
export default function GmailIntegration() {
    ...
    async function loadConfig() {
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (data.gmail) {
            setConfig({
              status: data.gmail.status || 'não_configurado',
              provider: data.gmail.provider || 'smtp_sec',
              notes: data.gmail.notes || ''
            });
          }
        }
      } catch (err) {
        console.error('Erro ao ler Gmail de settings/connectors:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFeedback({ type: 'success', message: 'Configurações de integração com Gmail salvas na nuvem com sucesso!' });
      ...
      console.error('Erro ao salvar Gmail:', err);
  ...
  const handleTestConnection = () => {
    ...
    const newLogs = [
      `[${timestamp}] Inicializando aperto de mão lógico com Gmail SMTP/OAuth (${config.provider})...`,
      `[${timestamp}] Testando conexão fática com o servidor SMTP: smtp.gmail.com:465...`,
      `[${timestamp}] Testando autenticação SSL segura...`,
      `[${timestamp}] Disparando e-mail de diagnóstico para o e-mail ativo do BOSS...`,
      ...
      <BossLayout>
        <div className="flex-1 p-6 md:p-10 flex items-center justify-center bg-gray-50 text-gray-400">
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-xs font-black uppercase tracking-widest text-gray-500">Sincronizando Gmail...</span>
          </div>
        </div>
      </BossLayout>
    );
    ...
    <BossLayout>
      <div className="flex-1 p-6 md:p-10 space-y-6 max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-3 mt-1.5">
              ...
              <div>
                <h2 className="text-lg font-black text-gray-900 tracking-tight uppercase">Gmail & E-mail</h2>
                <p className="text-xs text-gray-500 font-medium">Notificações e Backup de Comunicações via E-mail</p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-slate-900 border border-slate-950 p-6 rounded-[2rem] flex gap-3.5 text-slate-100 shadow-xl">
          <Info size={24} className="text-red-400 shrink-0 mt-0.5" />
          <div className="space-y-1">
            <p className="text-xs font-black uppercase tracking-wider text-red-400 font-mono">Conector E-mail Ativo • BOSS v5</p>
            <p className="text-xs text-slate-300 leading-relaxed font-semibold">
              Dispara e-mails formais com links de agendamentos, relatórios de integridade fáticos dos dados do cliente e faturas consolidadas de honorários.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSave} className="bg-white border border-gray-150 rounded-3xl p-6 md:p-8 space-y-6 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase text-gray-500 tracking-wider">Provedor SMTP / E-mail Selecionado *</label>
              <select
              ...
              >
                <option value="smtp_sec">SMTP Seguro (Gmail App Password)</option>
                <option value="gmail_oauth">Gmail API OAuth2 Flow</option>
                <option value="sendgrid">SendGrid transactional delivery</option>
              </select>
            </div>
          ...
          <div className="bg-slate-900 border border-slate-950 p-5 rounded-3xl text-slate-100 font-mono text-xs space-y-3 shadow-xl">
            <div className="flex justify-between items-center border-b border-slate-800 pb-2">
              <span className="text-[10px] text-slate-400 font-black uppercase tracking-wider flex items-center gap-2">
                ...
                <span>Terminal Simulator Logs — Gmail Gateway</span>
              </span>
              <button
                type="button"
