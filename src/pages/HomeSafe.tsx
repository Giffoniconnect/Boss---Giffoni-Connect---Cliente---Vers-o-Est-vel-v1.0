...
export default function HomeSafe() {
  ...
  useEffect(() => {
    // Safely attempt to read the cached settings link from localStorage/sessionStorage first
    try {
      const cachedLink = localStorage.getItem('boss_portal_link');
      if (cachedLink) {
        setPortalLink(cachedLink);
      }
    } catch (err) {
      console.warn('Could not read cached portal link:', err);
    }
  }, []);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-blue-100 via-gray-50 to-gray-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl w-full">
        <Link to="/boss-giffoni-clientes/login" className="group">
          <div className="h-full bg-white p-8 rounded-3xl border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-blue-50 transition-all">
        ...
        <a href={portalLink} target="_blank" rel="noopener noreferrer" className="group" id="portal-cliente-card-link">
          <div className="h-full bg-white p-8 rounded-3xl border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-blue-50 transition-all cursor-pointer">
            <div className="w-14 h-14 bg-blue-50 rounded-2xl flex items-center justify-center text-blue-600 mb-6 group-hover:bg-blue-600 group-hover:text-white transition-colors">
              <UserCircle size={28} />
            </div>
      ...
      <footer className="mt-20 text-gray-400 text-sm">
        &copy; {new Date().getFullYear()} Giffoni Connect. Todos os direitos reservados.
      </footer>
    </div>
  );
}
